...
```
Server URL: http://SERVER_IP:9005/api/v1/stream/stream
```
...
curl -v http://SERVER_IP:9005/api/v1/stream/stream

# Test with API key
curl -v -H "X-API-Key: dev-api-key-change-in-production" http://SERVER_IP:9005/api/v1/stream/stream

# Test POST with file upload
curl -v -X POST -H "X-API-Key: dev-api-key-change-in-production" \
  -F "image=@test.jpg" \
  http://SERVER_IP:9005/api/v1/stream/stream
```
...
```cpp
const char* SERVER_URL = "http://SERVER_IP:9005/api/v1/stream/stream";  // HTTP not HTTPS
```
